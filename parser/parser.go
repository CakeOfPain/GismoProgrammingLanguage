...
// skipNewlines consumes newline tokens from the reader.
// This is used to allow multi-line formatting within parenthesized expressions.
func skipNewlines(r *TokenReader) {
    for r.PeekNext(0).TokenType == tokentype.Newline {
        r.Next()
    }
}
...
func parseParentCall(r *TokenReader, left *SyntaxNode) *SyntaxNode {
    ...
    // FIX 2: Skip newlines immediately after opening parenthesis
    // Handles: func( \n arg )
    skipNewlines(r)
    ...
    if r.PeekNext(0).TokenType == tokentype.RParent {
        ...
        // FIX 3: Skip newlines after arguments but before closing parenthesis
        // Handles: func( arg \n )
        skipNewlines(r)
...
func parseCurlyParentCall(r *TokenReader, left *SyntaxNode) *SyntaxNode {
    ...
    // FIX 4: Optional consistency for empty blocks with newlines
    skipNewlines(r)
...
func parseLiteral(r *TokenReader) *SyntaxNode {
    ...
    case tokentype.LParent:
        r.Next() // consume '('
        skipNewlines(r) // Allow newline after '(' in grouped expression
        expr := parseExpression(r, 0)
        skipNewlines(r) // Allow newline before ')' in grouped expression
        if r.Next().TokenType != tokentype.RParent {
